...
VEHICLE_PARAMS = {
    ...
    # CORRECTED: Refrigerated vehicle costs are higher due to cooling requirements.
    'Refrigerated': {'speed_factor': 0.7, 'cost_rate_per_unit': 2.5, 'co2_rate_per_km': 0.35}, # Higher cost/CO2 for cooling
}

# Simulated Global Depot Locations 
GLOBAL_DEPOTS = {
    # India (IN)
    'Depot (Delhi, IN)': {'lat': 28.7041, 'lon': 77.1025, 'country_center': True},
    'Depot (Mumbai, IN)': {'lat': 19.0760, 'lon': 72.8777, 'country_center': True},
    'Depot (Bengaluru, IN)': {'lat': 12.9716, 'lon': 77.5946, 'country_center': True},
    'Depot (Kolkata, IN)': {'lat': 22.5726, 'lon': 88.3639, 'country_center': True},
    'Depot (Chennai, IN)': {'lat': 13.0827, 'lon': 80.2707, 'country_center': True},
    'Depot (Hyderabad, IN)': {'lat': 17.3850, 'lon': 78.4867, 'country_center': True},
    'Depot (Ahmedabad, IN)': {'lat': 23.0225, 'lon': 72.5714, 'country_center': True},
    'Depot (Pune, IN)': {'lat': 18.5204, 'lon': 73.8567, 'country_center': True},
    
    # North America (US/CA)
    'Depot (New York, US)': {'lat': 40.7128, 'lon': -74.0060, 'country_center': False},
    'Depot (Los Angeles, US)': {'lat': 34.0522, 'lon': -118.2437, 'country_center': False},
    'Depot (Chicago, US)': {'lat': 41.8781, 'lon': -87.6298, 'country_center': False},
    'Depot (Toronto, CA)': {'lat': 43.6532, 'lon': -79.3832, 'country_center': False},

    # Europe (UK/DE)
    'Depot (London, UK)': {'lat': 51.5074, 'lon': 0.1278, 'country_center': False},
    'Depot (Berlin, DE)': {'lat': 52.5200, 'lon': 13.4050, 'country_center': False},

    # Asia (SG/CN)
    'Depot (Singapore, SG)': {'lat': 1.3521, 'lon': 103.8198, 'country_center': False},
    'Depot (Shanghai, CN)': {'lat': 31.2304, 'lon': 121.4737, 'country_center': False},
}
...
def create_mock_data(n_rows=5000):
    ...
    # Randomly assign each delivery point to one of the global depots
    depot_list = list(GLOBAL_DEPOTS.keys())
    data['Cluster_Depot'] = np.random.choice(depot_list, n_rows)
    
    # Map the cluster center coordinates
    data['Center_Lat'] = data['Cluster_Depot'].apply(lambda x: GLOBAL_DEPOTS[x]['lat'])
    data['Center_Lon'] = data['Cluster_Depot'].apply(lambda x: GLOBAL_DEPOTS[x]['lon'])


    data['Latitude'] = data['Center_Lat'] + np.random.normal(loc=0, scale=0.5, size=n_rows).round(5)
    data['Longitude'] = data['Center_Lon'] + np.random.normal(loc=0, scale=0.5, size=n_rows).round(5)
    

    data.drop(columns=['Cluster_Depot', 'Center_Lat', 'Center_Lon'], inplace=True)
...
# CHART 1: Bar Chart - Orders by Status 
...
# CHART 2: Donut Chart - Customer Rating Breakdown 
...
# CHART 3: Scatter Plot - Fleet Health 
...
PRATICAL_UNIT_FACTOR = 150.0 
def calculate_distance(lat1, lon1, lat2, lon2):
    
    raw_distance = np.sqrt((lat1 - lat2)**2 + (lon1 - lon2)**2)
    
    return raw_distance * PRATICAL_UNIT_FACTOR 
...
def calculate_weighted_cost(distance, vehicle_type, W_cost, W_time, W_co2):
    ...
    time_cost = (distance * (1 / params['speed_factor'])) / TIME_SCALER
    ...
    NORM_DIST = 50.0 
    NORM_CO2 = 18.0 
    NORM_FINANCIAL = 125.0 

    NORM_TIME = 42.0 

    normalized_time_cost = time_cost / NORM_TIME
...
def find_optimized_route(df_stops, start_lat, start_lon, vehicle_type, W_cost, W_time, W_co2):
    ...
    # Calculate costs for the final segment (last delivery back to depot)
...
depot_lat_min = start_lat - 1.0
depot_lat_max = start_lat + 1.0
depot_lon_min = start_lon - 1.0
depot_lon_max = start_lon + 1.0

# Ensure we have enough stops to optimize
df_stops_to_optimize_local = df_filtered_region[
    (df_filtered_region['Latitude'] >= depot_lat_min) &
    (df_filtered_region['Latitude'] <= depot_lat_max) &
    (df_filtered_region['Longitude'] >= depot_lon_min) &
    (df_filtered_region['Longitude'] <= depot_lon_max)
].head(num_stops)

# Fallback if no local orders are found (unlikely but safe)
if df_stops_to_optimize_local.empty:
    st.warning(f"No local deliveries found near {selected_depot_name}. Showing random global orders instead.")
    df_stops_to_optimize = df_filtered_region.head(num_stops)
else:
    df_stops_to_optimize = df_stops_to_optimize_local

if df_stops_to_optimize.shape[0] < num_stops:
    st.info(f"Only {df_stops_to_optimize.shape[0]} orders available near {selected_depot_name}. Optimizing these.")
